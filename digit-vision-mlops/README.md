...
### Model uptime
- **UI:** "⏱️ Model Uptime" tab, or `GET /uptime` directly — shows service
...
2. On Render: **New → Web Service** → connect the repo → set
   **Dockerfile path** to `Dockerfile.api` → deploy. Note the resulting
   URL (this is your API URL).
3. **New → Web Service** again → **Dockerfile path** `Dockerfile.ui` →
   add environment variable `API_URL=<the API URL from step 2>` → deploy.
4. Paste both resulting URLs into the placeholders at the top of this
   README.
5. Re-run the Locust flood test in Section 6 against the **live** API URL
...
Record a screen capture **with your camera on**, showing:
1. A brief walkthrough of the repo structure and this README.
2. The notebook: preprocessing → training → the 4+ evaluation metrics →
   confusion matrix.
3. The UI: upload a single image and get a correct prediction.
4. The UI: bulk-upload a batch of new images, then trigger retraining, and
   show the model/metrics updating afterward.
5. (Optional but recommended) the Locust web UI mid-flood, and the
   resulting latency numbers at different container counts.

Upload to YouTube (unlisted is fine) and paste the link at the top of this
README.
...
- **Why `load_digits` instead of downloading an external image dataset?**
  This sandbox's network is restricted to package registries (PyPI/GitHub),
  not general dataset hosts. `load_digits` ships inside scikit-learn, so it
  needs no external download, and — critically — we still convert it into
  **real PNG files on disk**, so every downstream piece (upload, storage,
  augmentation, retraining, the `/predict` file-upload endpoint) genuinely
  operates on image files, not arrays. Swapping in a different image
  dataset later only requires replacing `build_image_dataset()`; nothing
  else in `src/`, `api/`, or `ui/` assumes anything about the specific
  dataset beyond "10 classes of 32×32 grayscale images."
- **Train/serve parity:** `preprocess_image()` in `src/preprocessing.py` is
  ...
  notebook's training code and the API's `/predict` endpoint call it.
